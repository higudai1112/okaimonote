...
export function ShoppingItemRow({ item, onToggle, onDelete }: Props) {
  return (
    <li className="flex items-center gap-3 bg-white px-4 py-3.5 rounded-2xl shadow-sm border border-orange-100 hover:shadow-md transition duration-150 ease-in-out w-full">
      {/* 左：円形チェックボタン（購入トグル） */}
      <button
        type="button"
      ...
      <button
        ...
        className="shrink-0 w-8 h-8 flex items-center justify-center text-gray-300 hover:text-red-400 hover:bg-red-50 rounded-lg transition"
      >
        ✕
      </button>
    </li>
